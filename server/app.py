...
class UserById(Resource):
    ...
    def get(self, id):
        ...
        if not user:
            user_dict = {'error': 'User not found'}
...
class JobListResource(Resource):
    ...
    def get(self):
        joblists = [{'id': job.id, 'title': job.title, 'description': job.description, 'location': job.location,
                    ...
                    for job in JobListing.query.all()]
        response = make_response(jsonify(joblists), 200)
        return response

    def post(self):
        data = request.get_json()
        new_job = JobListing(title=data['title'], description=data['description'], location=data['location'],
...
api.add_resource(JobListByIdResource, '/SearchJob/<int:id>')
